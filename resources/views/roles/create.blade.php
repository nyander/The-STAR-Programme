<x-app-layout>
    <div class="mt-6 rounded-lg max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <div class="flex justify-between items-center mb-10">
            <h2 class="font-medium text-lg">Create New Role</h2>
        </div>
        ...
        <form method="POST" action="{{ route('roles.store') }}" class="mt-4">
            ...
            <div class="name-section mb-4">
                ...
                <div class="field">
                    <div class="relative">
                        <input type="text" name="name" placeholder="Name" class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" value="{{ old('name') }}" />
                    </div>
                </div>
            </div>
                ...
                <div class="field">
                    @foreach($permission as $value)
                        <label class="flex items-center">
                            <input type="checkbox" name="permission[]" value="{{ $value->id }}" class="form-checkbox border-gray-300 rounded" />
